...
import { findAll } from '../../../services/TemporadaServices';
import { dateFormat } from "../../../utils";

export const TemporadaConsultaPage = () => {
    const [filter, setFilter] = useState<ITemporada>();
    const [descricao, setDescricao] = useState('');
    const [{ pageIndex, pageSize }, setPagination] = useState<PaginationState>({ pageIndex: 0, pageSize: 10 });
    const fetchDataOptions = { pageIndex, pageSize, filter };
    const { data, isFetching, isLoading, status } = useQuery(['temporadas', fetchDataOptions], () => findAll(fetchDataOptions));
    const pagination = useMemo(() => ({ pageIndex, pageSize }), [pageIndex, pageSize])
    const queryClient = useQueryClient();

    const columns = useMemo<ColumnDef<ITemporada>[]>(
        () => [
            { header: 'Descrição', accessorKey: 'descricao' },
            {
                header: 'Data de Início',
                accessorKey: 'horarioInicio',
                cell: (props: CellContext<ITemporada, any>) => {
            ...
            },
            {
                header: 'Ações',
                cell: (a) => {
                    const linkDetail = '/temporadas/detalhe/:id'.replace(':id', String(a.row.original.id))
                    const linkEdit = '/temporadas/alteracao/:id'.replace(':id', String(a.row.original.id))
                    const linkDelete = '/temporadas/exclusao/:id'.replace(':id', String(a.row.original.id))
                    return (
    ...
    const pesquisar = useCallback((e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setFilter({ descricao } as ITemporada);
